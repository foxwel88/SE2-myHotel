...
public class Promotion_stub implements Promotionblservice {
	...
	PromotionVO vo1 = new PromotionVO("i'm an id", PromotionType.SPECIALDATE.getString()
			, Calendar.getInstance().getTime(), Calendar.getInstance().getTime(), "goodhotel","address",2 , "", 7.5, "期末特惠");
	
	PromotionVO vo2 = new PromotionVO("i'm an id", PromotionType.SPECIALAREA.getString()
			, Calendar.getInstance().getTime(), Calendar.getInstance().getTime(), "","",2 , "仙林中心", 7.8, "仙林中心地区促销活动");
	
	PromotionVO vo3 = new PromotionVO("i'm an id", PromotionType.BIRTHDAYBONUS.getString()
